      ...
      for (var j = 0; j < functionsOnSuccess.length; j++) {
        for (var i = 0; i < functionsOnSuccess[j][1].length; i++) {
          if (functionsOnSuccess[j][1][i] == "response") {
            console.log(functionsOnSuccess[j][1][i]);
            functionsOnSuccess[j][1][i] = response;
            render=functionsOnSuccess[j][1][i];
            
          }
        }
        functionsOnSuccess[j][0].apply(this, functionsOnSuccess[j][1], render);
      }
      // render_componenets(response, functionsOnSuccess)
    }
  });
}
